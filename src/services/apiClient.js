    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          throw new ApiError({ detail: 'Session expired. Please login again.' }, response.status);
        }
        ...
        if (response.status === 403 && usedCorsProxy) {
          console.warn('403 Forbidden after CORS proxy - treating as authentication error');
          handleUnauthorized();
          ...
          throw new ApiError({ detail: 'Access forbidden. Please login again.' }, response.status);
        }
        ...
        if (!isOptionalEndpoint) {
          const errorMessage = data?.detail || data?.message || data?.title || `API Error: ${response.statusText}`;
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        const networkError = 'Network error: Unable to connect to the server. Please check your internet connection.';
